...
def solve_ibm_explicit(K, rhs_bc, areas, cells, tri_id_to_idx,
    ...
    while True:
        ...
        # --- Fluid solve (no IBM coupling for adiabatic BC)
        ...
        # --- Update IBM cells by mirroring (display only)
        T_full[i_idx] = T_full[f_idx][mirror_f_idx]
    ...
    # print("  Done.")
    rhs_steady_F = -rhs_bc[f_idx]
    T_steady_F   = spsolve(K_FF.tocsc(), rhs_steady_F)

    T_steady        = np.zeros(N)
    T_steady[f_idx] = T_steady_F
    # IBM cells in steady state: mirror from converged fluid
    T_steady[i_idx] = T_steady_F[mirror_f_idx]
    print("  Done.")
